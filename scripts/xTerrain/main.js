...
;;"一切权力归云梦所有";;
;;"https://github.com/xBoyMinemc/FlashFakePlayerPack";;
...
function 获取眼前的假人实体(逻辑主体,距离){
    ...
    return 实体们.find(({entity:实体})=>实体.hasTag(xboySign))?.entity;;"只返回一个假人";;
};
        ...
        工具人们.forEach((工具人,index)=>{
            ...
            try{
                if(工具人.hasTag(自动追击标识符)){
                    const 实体们 = 获取附近的非玩家实体(工具人,12,{families:["undead","monster"]});
...
world.beforeEvents.chatSend.subscribe( event => {
        ...
        //不会写，开摆--2023-07-21--写个烂东西留在这里
        if(message.startsWith(xboySimCmdHead)) return (event.setTargets([]),event.sendToTargets = flase );
})
...
world.afterEvents.chatSend.subscribe( event => {
    // try{
        ...
        if(消息=="交换背包" || 消息=="背包交换" ){
            const s = 眼前的工具人.getComponent("minecraft:inventory").container;                                     ;;"眼前的假人实体背包";;
            const p = sender.getComponent("minecraft:inventory").container;                                          ;;"你这个______的背包";;
        ...
        if(消息=="交换装备" || 消息=="装备交换" ){
            const s = 眼前的工具人.getComponent("minecraft:equipment_inventory");                                     ;;"眼前的假人实体背包";;
