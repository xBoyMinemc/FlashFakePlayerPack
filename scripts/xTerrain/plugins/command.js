...
function 获取假人实体眼前的方块的相对坐标(逻辑主体) {
    ...
    for (let i in 标签们)
        if (标签们[i].startsWith("#xyz#"))
            源坐标 = 标签们[i].replace("#xyz#", "").split("#");
        ...
        工具人们.forEach((工具人, index) => {
            ...
            try {
                ...
                if (工具人.getComponent("minecraft:health").currentValue <= 0) {
                    if (工具人.hasTag(AUTO_RESPAWN_SIGN))
                        工具人.respawn();
                    return;
                }
                ;
            ...
            ;
            try {
                if (工具人.hasTag(自动攻击标识符) && 获取眼前的实体(工具人, 4).name != "")
                    工具人.attackEntity(获取眼前的实体(工具人, 4));
            }
            catch (e) { }
            ;
