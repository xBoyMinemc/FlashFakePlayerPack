...
function 获取眼前的假人实体(逻辑主体, 距离) {
    ...
    return 实体们.find(({ entity: 实体 }) => 实体.hasTag(SIGN.YUME_SIM_SIGN))?.entity;
    ;
    "只返回一个假人";
    ;
}
...
function 获取附近的玩家实体(逻辑主体, 距离) {
    ...
    "距离";
    ;
    吃个桃桃.location = new Location(逻辑主体.location.x, 逻辑主体.location.y, 逻辑主体.location.z);
    ;
    ;
    "中心坐标-ri泥god";
    ;
    吃个桃桃.type = "minecraft:player";
    ;
    ;
    "排除掉的实体类型";
    ;
...
function 获取附近的非玩家实体(逻辑主体, 距离, 昊京牌过滤器) {
    ...
    吃个桃桃.excludeTypes = ["minecraft:player", "minecraft:arrow", "minecraft:xp_orb", "minecraft:item"];
    ;
    ;
    "排除掉的实体类型";
    ;
...
world.events.tick.subscribe(() => {
    ...
    try {
        ...
        for (const index in 工具人们) {
            ...
            ;
            try {
                if (工具人.hasTag(SIGN.AUTO_ATTACK_SIGN) && 获取眼前的实体(工具人, 4).name != "")
                    工具人.attackEntity(获取眼前的实体(工具人, 4));
            }
            catch (e) { }
            ;
            ;
            ...
            ;
            try {
                if (工具人.hasTag(SIGN.AUTO_JUMP_SIGN))
                    工具人.jump();
            }
            catch (e) { }
            ;
